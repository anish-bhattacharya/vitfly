...
### 5.3a Velocity Scaling: 5m/s vs 7m/s @ 60m

The teacher was trained for 7m/s flight, so we tested both teacher and E Distill at 7m/s on the 60m track to evaluate speed robustness:

| Model @ 60m | 5m/s | 7m/s | Δ | 5m/s Time | 7m/s Time |
|-------------|------|------|---|-----------|-----------|
| Teacher ViT+LSTM | 2 crashes | **5 crashes** ❌ | **+3 crashes** | 12.24s | 8.94s |
| **E Distill** (DecisionMamba) | **1 crash** ✅ | **1 crash** ✅ | **0** 🏆 | 12.23s | 8.78s |

Key findings:
- **Teacher degrades at its "native" speed**: 2→5 crashes at 7m/s. This is unexpected — the teacher was trained for 7m/s but performs worse at higher speed on this obstacle course.
- **E Distill is speed-robust**: 1 crash at BOTH speeds. The distilled Mamba model generalizes better across velocities than the teacher.
- **E Distill beats teacher at both speeds**: At 5m/s (1 vs 2), at 7m/s (1 vs 5). This strongly reinforces that distillation produces superior policies.

### 5.3b Born-Again Distillation (B+ → E)

A born-aging experiment was conducted (by the training pipeline) where B+ Distill (best performer) was used as the teacher for E (DecisionMamba):

| Metric | ViT+LSTM Teacher | B+ Teacher (born-again) | Δ |
|--------|-----------------|------------------------|---|
| val_distill_gap | 0.0172 | **0.0037** | ↓**4.4×** |
| val_loss_gt | 0.0188 | **0.0172** | ↓ |
| val_score | 0.0274 | **0.0190** | ↓31% |

The born-again distill_gap (0.0037) is 4.4× smaller than cross-architecture distillation, confirming that same-architecture transfer is more efficient.

> ⏳ **60m simulation verification pending**: The born-again checkpoint (`bornagain_best_model.pth`) was generated by the training pipeline but not committed to the repository (only `bornagain_summary.json` was pushed). Simulation verification at 60m is needed to confirm whether the improved val metrics translate to lower crash rates.

### 5.4 20m vs 60m Comparison
...
### 5.5 Velocity Scaling: Initial Tests at 7m/s (20m track)
