...
def check_gradients(model):
    ...
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            if np.isnan(norm): has_nan = True
            if np.isinf(norm): has_inf = True
    total_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 10.0).item()
    return total_norm, grad_norms, has_nan, has_inf

def train_epoch(model, loader, opt, crit, device, epoch):
    model.train(); total_loss, n = 0, 0
    grad_stats = {'total': [], 'vmamba': [], 'lstm': []}
    for i, (d, v, q, t) in enumerate(loader):
        ...
        # 梯度检查
        total_norm, norms, has_nan, has_inf = check_gradients(model)
        grad_stats['total'].append(total_norm)
        if 'vmamba' in str(model.named_parameters()): grad_stats['vmamba'].append(total_norm)
        
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()
        total_loss += loss.item(); n += 1
        if i % 50 == 0: print(f"  Epoch {epoch}, Batch {i}/{len(loader)}, Loss: {loss.item():.4f}, Grad: {total_norm:.4f}", flush=True)
    return total_loss / n, grad_stats
...
def main():
    ...
    p.add_argument('--exp_id', default='A_exp04_gradcheck')
    ...
    for epoch in range(args.epochs):
        start = time.time()
        train_loss, grad_stats = train_epoch(model, train_loader, opt, crit, device, epoch+1)
        val_loss = validate(model, val_loader, crit, device)
        sched.step()
        
        avg_grad = np.mean(grad_stats['total']) if grad_stats['total'] else 0
        all_grad_stats.append({'epoch': epoch+1, 'avg_grad': avg_grad, 'train_loss': train_loss, 'val_loss': val_loss})
        
        print(f"Epoch {epoch+1}/{args.epochs} - Train: {train_loss:.4f}, Val: {val_loss:.4f}, Avg Grad: {avg_grad:.4f}, LR: {sched.get_last_lr()[0]:.6f}, Time: {time.time()-start:.1f}s", flush=True)
    ...
    # 保存梯度统计
    np.save(f"/root/catkin_ws/src/vitfly/experiments/mamba_branches/branch_A_vmamba_lstm/logs/{args.exp_id}_gradstats.npy", np.array(all_grad_stats))
