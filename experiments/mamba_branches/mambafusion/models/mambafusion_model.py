...
class MambaFusion(nn.Module):
    def __init__(self, config=None):
        ...
        self.coarse_ssm = CoarseSSM(dim=512, d_state=64)
        self.fine_ssm = FineSSM(dim=512, d_state=32)
    ...
    def forward(self, X):
        ...
        coarse_out, _ = self.coarse_ssm(vis_feat)
        fine_out, _ = self.fine_ssm(self.ssm_norm(coarse_out))
        state_in = torch.cat([fine_out, X[1] * 0.1, X[2]], dim=1).float()
