...
def compute_command_vision_based(state, orig_img, prev_img, desiredVel, trained_model, hidden_state, seq_len=1):
    ...
    if seq_len > 1 and isinstance(orig_img, np.ndarray) and orig_img.ndim == 3:
        imgs_resized = np.stack([cv2.resize(f, (w, h)) for f in orig_img])
        imgs_tensor = torch.from_numpy(imgs_resized).float().unsqueeze(1)
        B, S = 1, seq_len
    else:
        imgs_tensor = None
        S = 1
    ...
    _is_branch_bce = _class in ('MambaVisionSSMNet', 'CNNMamba3Net', 'STHMambaNet', 'DecisionMambaNet', 'BPlusModel', 'MambaFusion', 'EssmNet', 'BranchFModel')
