...
class SpatiotemporalTransformer(nn.Module):
    """
    Reduced Capacity Spatio-Temporal Transformer
    """
    def __init__(self, embed_dim=256, depth=2, heads=4, num_classes=3):
        super().__init__()
        self.name = "SpatioTemporalViT"
        ...
        self.decoder = nn.Linear(4608, embed_dim)
        
        # === Temporal Transformer (Reduced) ===
        encoder_layer = nn.TransformerEncoderLayer(          # ← Fully qualified
            d_model=embed_dim,
            nhead=heads,
        ...
        # Final Head
        self.nn_fc1 = nn.utils.spectral_norm(nn.Linear(embed_dim + 5, 128))
        self.nn_fc2 = nn.utils.spectral_norm(nn.Linear(128, num_classes))
    ...
    def forward(self, X):
        ...
        # Metadata + Head
        meta = torch.cat([X[1]/10.0, X[2]], dim=1).float()
