...
Given that B+ and E achieved the best distillation results, we conducted born-again distillation: using B+ (distilled checkpoint, 1 crash at 60m) as the teacher to further distill E. The same-architecture transfer (Mamba $\rightarrow$ Mamba) achieved a distill gap of 0.0037 versus 0.0172 for cross-architecture (ViT $\rightarrow$ Mamba), a 4.4$\times$ improvement. Feature alignment loss dropped from 10.47 to 0.12, confirming that same-architecture feature spaces are inherently more compatible. This checkpoint is pending 60m simulation verification.

\subsection{The 20m Artifact}
Initial testing at 20m (1/3 of the course) falsely suggested distillation degrades 5/6 branches. Full 60m evaluation revealed the opposite. This highlights the critical role of thorough benchmarking.
...
Simulation-only evaluation; real-world flight testing at 7m/s pending. Loss weights not per-branch optimized. Single obstacle course configuration. All results use single-step prediction ($T=1$); multi-step sequence training---which better leverages the LSTM and SSM temporal heads---is a natural extension that may further improve performance.
