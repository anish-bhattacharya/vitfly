...
Contrary to initial expectations, neither architectural similarity to the teacher nor SSM purity predicts distillation success. The two best results (DecisionMamba, MambaVision) share one critical property: their visual encoders preserve spatial structure. DecisionMamba uses a lightweight CNN (455K params) that processes images through spatial convolutions; MambaVision's encoder combines depthwise convolutions with a simplified MLP path---the convolution is the dominant component \cite{hatamizadeh2024mambavision}. Both produce features in a spatially organized representation that can be aligned to the teacher's ViT features via MSE loss. This aligns with recent hybrid architectures (VCMamba \cite{vcmamba2025}) that similarly place CNN blocks in early stages for local feature extraction and SSM blocks in later stages for global context.

In contrast, architectures that flatten spatial structure via selective-scan SSM encoders (CoarseSSM, SS2D) produce features in a sequential space that simple MSE alignment cannot bridge. However, this does not indicate fundamental incompatibility---specialized techniques such as direction-aware distillation \cite{transmamba2025} and attention bridges \cite{wang2025cab} have shown success with such encoders. Our results specifically identify the boundary conditions for \emph{simple} MSE-based distillation: it works when the encoder preserves spatial organization; it fails when the encoder uses sequential flattening.

\subsection{Spatial Structure vs SSM Design}
We verified this hypothesis through a controlled experiment: replacing DecisionMamba's CNN encoder with a lightweight selective-scan SSM (CoarseSSM, 1.67M total params) at the same feature dimension (256-dim) caused the feature alignment loss to dominate (10.58 vs 0.85 for GT), regardless of feature alignment weight ($\alpha=1.0$ vs $\alpha=0.3$), initialization (random vs BC-pretrained), or distillation variant.

Crucially, this does NOT imply that SSM-based encoders are fundamentally incompatible with cross-architecture distillation. Recent work has demonstrated successful Transformer-to-Mamba distillation through specialized techniques: TransMamba \cite{transmamba2025} proposes adaptive multi-directional distillation that aligns features along each scanning direction separately, and CAB \cite{wang2025cab} introduces a lightweight MLP-based attention bridge that maps Transformer Q,K projections to Mamba B,C. Our results specifically isolate the failure of \emph{simple MSE alignment on pooled features} — the most straightforward distillation approach and a common baseline in the literature. This establishes a boundary condition: spatially organized encoders (CNN, CNN+MLP hybrids) work with basic MSE alignment, while SSM-based encoders require more sophisticated, direction-aware distillation methods. The practical implication for robotics practitioners is clear: if using simple MSE distillation, prefer CNN-based encoders over pure SSM encoders for cross-architecture transfer.
