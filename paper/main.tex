...
\title{Mamba Architecture Advantages in Imitation Learning\\and Cross-Architecture Knowledge Distillation for\\Vision-Based Quadrotor Obstacle Avoidance}
...
\begin{itemize}
    \item \textbf{First systematic comparison of six SSM-based architectures for vision-based quadrotor control.} We evaluate VMamba+LSTM, MambaVision+SSM, CNN+Mamba-3, STH-Mamba, DecisionMamba, and a MambaVision+Mamba-3 hybrid across 60m obstacle courses at multiple speeds, establishing DecisionMamba (2.19M params) as the best architecture with 7.1ms inference (21\% faster than ViT+LSTM), and unique speed-robustness (1 crash at both 5m/s and 7m/s). Notably, DecisionMamba's encoder is a lightweight CNN (455K params), not a selective-scan SSM --- its advantage comes from temporal SSM processing, not from its visual encoder.

    \item \textbf{A cross-architecture distillation framework that produces Mamba students surpassing the ViT+LSTM teacher.} Multi-stage alignment (feature alignment + output distillation + GT supervision) achieves \textbf{1 crash at 60m} for both DecisionMamba (E) and a MambaVision-based hybrid (B+), outperforming their BC baselines (3 crashes) and the teacher (2 crashes at 5m/s, 5 crashes at 7m/s).

    \item \textbf{Feature-alignment distillation requires spatial structure in the student encoder.} The two best distillation results (E, B+) both use encoders that preserve spatial feature organization --- CNN for E, CNN+MLP for B+. Attempts to replace E's CNN encoder with a true selective-scan SSM (CoarseSSM) cause feature alignment to fail (gt\_loss 0.02 → 0.83), because the scan operation flattens spatial structure into a sequence, creating a representational gap that MSE cannot bridge. This finding corrects a common misconception: MambaVision's distillation success is due to its CNN path, not its SSM components.

    \item \textbf{A systematic ablation of alternative approaches revealing clear boundary conditions.} Data augmentation helps CNN-based encoders but degrades pure SSM ones; multi-step prediction underperforms single-step; born-again and loss-weight tuning fail to improve over cross-architecture transfer. These negative results prevent redundant exploration by future researchers.
\end{itemize}
