...
\begin{itemize}
    \item \textbf{First systematic comparison of six SSM-based architectures for vision-based quadrotor control.} We evaluate VMamba+LSTM, MambaVision+SSM, CNN+Mamba-3, STH-Mamba, DecisionMamba, and a MambaVision+Mamba-3 hybrid across 60m obstacle courses at multiple speeds, establishing DecisionMamba (pure SSM) as the best architecture with 2.19M params, 7.1ms inference (21\% faster than ViT+LSTM), and unique speed-robustness (1 crash at both 5m/s and 7m/s).

    \item \textbf{A cross-architecture distillation framework that produces Mamba students surpassing the ViT+LSTM teacher.} Multi-stage alignment (feature + output + GT) achieves \textbf{1 crash at 60m} for B+ (2.55M) and E (2.19M), outperforming BC baselines (3 crashes) and the teacher (2 crashes at 5m/s, 5 crashes at 7m/s).

    \item \textbf{An SSM design principle: spatial structure is required for feature-alignment distillation.} MambaVision's hybrid encoder succeeds in cross-architecture distillation not because of its SSM components, but because its CNN path preserves spatial structure. Pure selective-scan encoders (CoarseSSM) flatten spatial dimensions and produce features that cannot be aligned to ViT features via MSE loss. This explains why MambaVision-based students (B+) match DecisionMamba (E) at 1 crash, while CoarseSSM-based students fail to converge. For successful distillation, the student encoder must maintain spatial organization --- a trivial insight often overlooked in SSM vision architectures.
