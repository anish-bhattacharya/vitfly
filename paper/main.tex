...
\begin{itemize}
    ...
    \item \textbf{An SSM design principle: selective scan must be present in both vision encoder and temporal head.} Branches with SSM at both ends achieve 1 crash; those with non-SSM components at either end underperform (2-3 crashes). This provides a design guideline for future SSM-based control architectures.

    \item \textbf{A systematic ablation of alternative approaches revealing clear boundary conditions.} Data augmentation helps MambaVision encoders but degrades CNN-based ones; multi-step prediction underperforms single-step; born-again and loss-weight tuning fail to improve over cross-architecture transfer. These negative results prevent redundant exploration by future researchers.
\end{itemize}
