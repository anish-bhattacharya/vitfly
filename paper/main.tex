...
\begin{itemize}
    \item \textbf{Mamba architectures demonstrate inherent advantages for imitation learning in quadrotor control.} Across six diverse SSM variants, we find that SSM purity correlates with BC performance. The pure-SSM DecisionMamba (2.19M params) achieves 7.1ms inference (21\% faster than ViT+LSTM) and is the only model with speed-robust performance (1 crash at both 5m/s and 7m/s).
    \item \textbf{Cross-architecture knowledge distillation (ViT+LSTM $\rightarrow$ Mamba) further improves performance, surpassing the teacher.} Our multi-stage distillation framework transfers visual features from a frozen ViT+LSTM to Mamba students, achieving \textbf{1 crash at 60m} for B+ (2.55M) and E (2.19M) --- outperforming both their BC baselines (3 crashes) and the teacher (2 crashes, 3.56M).
    \item \textbf{Systematic ablation of alternative approaches reveals clear boundaries.} Data augmentation (horizontal flip + noise) helps attention-based encoders (B/B+) but degrades CNN-based ones (C/D/E). Multi-step prediction (seq\_len $>$ 1) universally underperforms single-step. Loss weight tuning has negligible impact. Born-again and iterative distillation fail to improve over cross-architecture transfer. These negative results establish important boundary conditions for future work.
\end{itemize}
